...
ConfigNotConsistent = Class.new(StandardError)
ConfigDirty = Class.new(StandardError)


def get_active_ports()
    ports = []
    recs=`ps -ef |grep redis`.split('[cluster]');
    recs.each{|rec|
      next if ! rec.index("*")
      ports << rec.split("*:")[1].strip()
    }
    return ports
...
def xputs(s)
    print DateTime.now().iso8601[0..18]+" "+s+"\n"
end
...
class ClusterNode
    ...
    def is_traffic_node
      if $debug
           return !(@info[:port] == "9000" || @info[:port] == "9001")
      else
          return @info[:port] == "6378"  || @info[:port] == "6379" 
      end      
    end
    ...
    def connect(o={})
        ...
        begin
            ...
            xputs "[ERR] Sorry, can't connect to node #{self}, exit now"
            exit 1 if o[:abort]
            @r = nil
        end
        xputs "Connect to node #{self} OK"
    end
    ...
    def load_info(o={})
        self.connect(:abort => true)
        nodes_cmd_output = @r.cluster("nodes")
        nodes = nodes_cmd_output.split("\n")
        puts "#{@info[:host]}:#{@info[:port]} cluster nodes:\n#{nodes_cmd_output}"
        nodes.each{|n|
            split = n.split

            #78e18af21aadd9d631d87fd924956fe2e3776a48 127.0.0.1:7000 master - 0 1483667282258 1 connected 0-5460
            name,addr,flags,master_id,ping_sent,ping_recv,config_epoch,link_status = split[0..7]
            slots = split[8..-1]
            info = {
                :name => name,
                :addr => addr,
    ...
    def remove_fail_nodes()
        self.connect(:abort => true)
        nodes = @r.cluster("nodes").split("\n")
        nodes.each{|n|
            split = n.split

            #78e18af21aad 127.0.0.1:7000 master - 0 1483667282258 1 connected 0-5460
            name,addr,flags,master_id,ping_sent,ping_recv,config_epoch,link_status = split[0..7]
            slots = split[8..-1]
            info = {
                :name => name,
            ...
            next if info[:flags].index("myself")

            if info[:link_status] == "connected"
                next
            end

            info_dirty = false
            if info[:flags].index("fail") || info[:flags].index("fail?") || info[:flags].index("noaddr")
              # don't remove fail nodes too early, otherwise node in noaddr status and can't forget any more
                if !$sleeped
                    xputs ">>> sleep #{MaxFailTime} before remove"
                    sleep MaxFailTime
                    $sleeped = true
                end

                if(@info[:name]==info[:name])
                    xputs "[ERR] forgetting myself"
                else
                    xputs "forgetting #{info[:name]}"
                    begin
                        @r.cluster("FORGET",info[:name])
                        info_dirty = true
                    rescue => e
                        if /ERR Can't forget/.match(e.to_s) != nil
                            xputs "[ERR] #{e.to_s}"
                        end
                    end
                end
            ...
            if info_dirty
                # don't sleep, so info can sync quickly
                # sleep ClusterSyncTime
                load_info(:getfriends => false)
            end
        }
    ...
    def get_config_signature
        ...
        @r.cluster("nodes").each_line{|l|
            next if l.index("fail") || l.index("fail?") || l.index("disconnected")
            s = l.split
...
class RedisTrib
    ...
    def add_node(node)
        if @nodes.index(node)
            raise "[ERR], node already exist!"
        else
            @nodes << node
        end
        raise "ERR exceed max nodes" if @nodes.length > @iports.length
    end
    ...
    def show_cluster_info
        ...
        keys = 0
        @nodes.each{|n|
            if n.has_flag?("master") && n.is_traffic_node()
    ...
    def check_slots_coverage
        ...
        if slots.length == ClusterHashSlots
            ...
            cluster_error \
                "[WARN] Not all #{ClusterHashSlots} slots are covered by nodes."
...
#    def del_slot_on_all_nodes(slot)
#        @nodes.each{|n|
#            begin
#                res = n.r.cluster("delslots",slot)
#            rescue => e
#                puts "#{n}: #{e}"
#            end
#        }
#    end
    ...
    def fix_slots_coverage
        ...
        # don't fix slots on new nodes(move_slots_to_empty_node proc will do that),
        # to make sure existing nodes get average slots
        masters = @nodes.select{|n| n.has_flag?("master") && n.is_traffic_node() && n.slots.length>0}
        if masters.length == 0
            puts "initial assign slots in cluster"
            masters = @nodes.select{|n| n.has_flag?("master") && n.is_traffic_node()}
            if masters.length == 0
                # all are slaves, need reset cluster later
                raise "no masters with slots >0"
            end
        end

        max = not_covered.length/masters.length
        n=masters.shift()
        count = 0
        not_covered.each{|slot|
                xputs ">>> adding slot #{slot} to #{n.info[:host]}:#{n.info[:port]}"
                begin
                    n.r.cluster("addslots",slot)
                rescue => e
                    if e.to_s =~ /is already busy/
                        raise ConfigDirty, "fail to add slot, some slave should have become master and own that slot, or disconnected node own it, get latest config and retry later"
                    else
                        raise e
                    end
                end
    ...
    def check_config_consistency
        if !is_config_consistent?
            xputs "[WARN] sleep 30s to check consistent again"
            sleep 30
            refresh_nodes_info()
            if !is_config_consistent?
                cluster_error "[ERR] nodes don't agree about configuration!"
                raise ConfigNotConsistent, "nodes don't agree about configuration!"
            end
        end
    ...
    def is_config_consistent?
        signatures=[]
        @nodes.each{|n|
            next if n.has_flag?("slave")    #david, skip slaves
            puts "#{n.info[:host]}:#{n.info[:port]} signature: "+n.get_config_signature
            signatures << n.get_config_signature
        }

        return true if signatures.length == 0
    ...
    def wait_cluster_join
        ...
        while !is_config_consistent?
            ...
            sleep 1
            count+=1
            raise "wait too long to join" if count>60
        end
        print "\n"
    end
    ...
    def load_cluster_info_from_node()
        reset_nodes()
        @errors=[]

        @iports.each{|iport|
          node = ClusterNode.new(iport)
          node.connect(:abort => true)
          node.assert_cluster
          next if !node.r
          node.load_info(:getfriends => false)  #david don't load friend, it have problem
          node.remove_fail_nodes()
          add_node(node)
        }
...
#        }
    ...
    def refresh_nodes_info()
        # don't remove sleep, nodes need time to exchange cluster info
        sleep ClusterSyncTime
        @errors=[]
        xputs ">>> refreshing nodes info..."
        @nodes.each{|node|
          node.load_info(:getfriends => false)  #david don't load friend, it have problem
          node.remove_fail_nodes()
          populate_nodes_replicas_info
    ...
    def show_reshard_table(table)
        table.each{|e|
            puts "    Move slot #{e[:slot]} from #{e[:source].info[:name]}"
        }
    end
    ...
    def move_slot(source,target,slot,o={})
        ...
        if !o[:quiet]
            xputs "Moving slot #{slot} from #{source} to #{target}"
        end
    ...
    def rebalance_cluster_cmd(argv,opt)
       ...
       # Assign a weight to each node, and compute the total cluster weight.
        ...
        #david: this code will loop forever
    ...
    def reshard_cluster(opt)
        ...
        if sources == "all"
            ...
            @nodes.each{|n|
                ...
                next if n.has_flag?("slave")
                next if (!n.is_traffic_node())
                next if n.slots.length <= numslots
                sources << n
            }
        end
    ...
    def get_excessive_slaves()
        excessive_slaves = []
        masters_excess_replicas = @nodes.select{|n| n.has_flag?("master") && n.info[:replicas].length>1}
        masters_excess_replicas.each{|n|
            excess_slaves=n.info[:replicas][0..-2]
            excess_slaves.each{ |n|
              excessive_slaves << n
              xputs ">>> detect excessive slave #{n.info[:host]}:#{n.info[:port]}"
            }
        }
        return excessive_slaves
    end

    def match(s_cand,m_cand)
      if s_cand.length <1 ||  m_cand.length<1
          return false
      end
      ...
      m_cand.each{|m|
          s_cand.each{ |s|
              next if m==s
              if $debug
                canreplicate=(m.info[:port]!=s.info[:port])
              else
                canreplicate=(m.info[:host]!=s.info[:host])
              end

              if canreplicate
                  xputs ">>> #{s.info[:host]}:#{s.info[:port]} is replicating #{m.info[:host]}:#{m.info[:port]}"
                  begin 
                      s.r.cluster('REPLICATE',m.info[:name])
                  rescue => e
                      if e.to_s =~ /Unknown node/
                          # I don't know the target nodes
                          xputs "[WARN] #{e.to_s}"
                      else
                          raise e
                      end
                  end
                  # in case s_cand and m_cand are same
                  s_cand.delete(s)
                  s_cand.delete(m)
                  m_cand.delete(s)
                  m_cand.delete(m)
      ...
      #need to repopulate replicate info

      return  false
    end

    def reset_slaves_no_master()
        @nodes.each{|n|
            next if n.info[:replicate]== false
            masterid = n.info[:replicate]
            masters=@nodes.select{|n| n.has_flag?("master") && n.info[:name]==masterid}
            if masters.length == 0
                xputs "[WARN] slave #{n.info[:name]} masters does not exist!reset it"
                n.r.cluster("RESET","HARD")
            end
        }
    end
    ...
    def auto_replicate()
        ...
        m1 = @nodes.select{|n| n.is_traffic_node() && n.has_flag?("master") && n.info[:replicas].length==0 && n.info[:slots].length>0}
        s1 = @nodes.select{|n| n.is_traffic_node() && n.has_flag?("master") && n.info[:replicas].length==0 && n.info[:slots].length==0}
        ...
        if s1.length > 1
            # we hope it replicate as:
            # Host1 6378 -> Host3 6379
            # Host2 6378 -> Host1 6379
            # Host3 6378 -> Host2 6379
  
            m2 = s1.select{|n| n.info[:port].to_i%2==0}   # try to make 6378 as master, 6379 as slave           
            s2 = s1.select{|n| n.info[:port].to_i%2==1}   

            if s2.length > 1
                s2 = s2.unshift(s2.pop())
            end     
    ...
    def move_slots_to_empty_node()
        xputs "================================================="
        xputs "*          move slots to empty node             *"
        xputs "================================================="

        # no need to check cluster as fix_cluster have do that
        masters = @nodes.select{|n| n.has_flag?("master") && n.is_traffic_node()}
        
        @nodes.each{|node|
            next if (!node.is_traffic_node())
            next if node.has_flag?("slave") || node.slots.length>0 
            puts "try to move slots to nodes:#{node.info[:host]}:#{node.info[:port]}"
            opt = {}
            opt['slots']  = (ClusterHashSlots/masters.length.ceil).ceil
            opt['to'] = node.info[:name]
            opt['from'] = 'all'
            opt['yes'] = true
            reshard_cluster(opt)
        }
    ...
    def graceful_shut_down(myiport)
        init_redis_list()
        xputs ">>> graceful shut down start"
        node = ClusterNode.new(myiport)
        node.connect(:abort => true)
        node.load_info(:getfriends => false)
        call_cluster_cmd([@iports[0],"CLUSTER","FORGET",node.info[:name]],{})
 
        begin
          node.r.client.call(["SHUTDOWN","NOSAVE"])
        rescue => e
          # This means Redis has probably exited.
        end
    end

    def join_all_to_cluster()
        xputs ">>> join all nodes into a cluster"
        myself = @iports[0];
        node = ClusterNode.new(myself)
        node.connect(:abort => true)

        @iports.each{|iport|
          next if iport==@iports[0]
          ip,port = iport.split(':')
          node.r.cluster('meet',ip,port)
        }
    ...
    def destroy_cluster()
        xputs ">>> Reseting cluster..."
        if $debug 
            exit 1
        end
        call_cluster_cmd([@iports[0],"CLUSTER","RESET","HARD"],{})
        @config_dirty_count = 0
        @destroy_time << Time.now.nsec

        if @destroy_time[-3]!=nil && (Time.now.nsec-@destroy_time[-3]<180)
            xputs "[ERR] reset frequently! exit"
            exit 1
        end
    end
    ...
    def create_cluster()
      ...
      if @iports.length%2==0
          create_cluster_cmd(@iports,{"replicas"=>1})
          @replication_pool=[]+@iports
      else
          create_cluster_cmd(@iports[0..-2],{"replicas"=>1})
          @replication_pool=[]+@iports[0..-2]
      end
    end
    ...
    def init_redis_list()
      @iports=[]
      @ssmiports=[]
      at_least_one_traffic_node = false;
      if $debug
           ports= get_active_ports()
           ports.each{|port|
              @iports << "127.0.0.1:#{port}"           
              if port=="9000" || port=="9001"
                  at_least_one_traffic_node = true
              end
           }
      else
          begin
              uri = URI('http://127.0.0.1:8500/v1/health/service/redis?passing&stale')
              services = JSON.parse(Net::HTTP.get(uri))
              services.each{|service| 
                  @iports << "#{service['Service']['Address']}:#{service['Service']['Port']}"                            
                  if service['Service']['Port'] == 6378 || service['Service']['Port'] == 6379
                      at_least_one_traffic_node = true
                  end
              }
          rescue => e
              xputs "[ERR] exception: #{e}"
          end
      end

      @iports = @iports.sort
      puts "redis list:#{@iports.inspect}"
      return at_least_one_traffic_node
  end
  
  
  def maintain()
    xputs "start maintain"
    @config_dirty_count = 0
    ...
    loop do
      begin
        $sleeped = false
        if !init_redis_list()
          xputs "no redis server is up:#{@iports.inspect}"
          exit 0
        end
        #step1 join all
        join_all_to_cluster()
  
        #step2 load cluster info
        load_cluster_info_from_node()
  
        #step3 assign roles
        auto_replicate()
        
        #step4: make sure all slots are covered
        fix_cluster_cmd(@iports[0],{})
        sleep ClusterSyncTime
  
        #step5: move slots to new nodes
        move_slots_to_empty_node()
        
        #step6: reset slave whose master does not exist any more
        reset_slaves_no_master()
        xputs "[OK] maintain finished successfully"
        if $debug
            return true
        else
            exit 0
        end
      rescue ConfigNotConsistent => e
        xputs "[ERR] exception: #{e}"
        puts e.backtrace
        destroy_cluster()
        exit 1
      rescue ConfigDirty => e
        xputs "[WARN] exception: #{e}"
        @config_dirty_count += 1
        xputs "[WARN] config dirty #{@config_dirty_count} times"
        # sleep long enough to get info sync
        puts "sleep #{MaxDirtyTime}s"
        sleep MaxDirtyTime 
        if(@config_dirty_count > 3)
          xputs "[ERR] config dirty times exceed max"
          destroy_cluster()
          exit 1
        end
        ...
        xputs "[ERR] exception: #{e}"
        puts e.backtrace
        xputs ">>> exit due to network error"
        exit 1
      rescue => e
        if e.to_s =~ /loading the dataset in memory/
          xputs ">>> slave is loading data, sleep 120s and exit"
          sleep 120
          exit 0
        end
               
        xputs "[ERR] unrecoverable exception: #{e}"
        puts e.backtrace
        destroy_cluster()
        exit 1
      end
...
if  ARGV.length == 0
    puts "ruby cluster-tool.rb debug/production ctrl"
    puts "ruby cluster-tool.rb debug/production shutdown 6378/6379"
elsif ARGV[1] == "shutdown"
    $debug = (ARGV[0] == "debug")
    if $debug
        ClusterHashSlots = 50
    end    
    ip = `getip internal-data-network`
    rt = RedisTrib.new
    rt.graceful_shut_down("#{ip}:#{ARGV[2]}")
elsif ARGV[1] == "ctrl"
    $debug = (ARGV[0] == "debug")
    if $debug
        ClusterHashSlots = 50
